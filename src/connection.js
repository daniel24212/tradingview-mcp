...
function isConnectionError(err) {
  ...
  return msg.includes("econnreset") || msg.includes("econnrefused") || msg.includes("websocket") || msg.includes("session closed") || msg.includes("target closed") || msg.includes("connection closed");
}
...
export async function getClient() {
  if (client) {
    try {
      ...
      const _liveness = client.Runtime.evaluate({ expression: "1", returnByValue: true });
      const _timeout = new Promise((_, r) => setTimeout(() => r(new Error("liveness timeout")), 3000));
      await Promise.race([_liveness, _timeout]);
      return client;
    } catch {
      client = null;
      targetInfo = null;
