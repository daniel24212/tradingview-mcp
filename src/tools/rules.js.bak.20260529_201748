...
function buildVerdict(results) {
  ...
  for (const r of results) { if (r.signal === 'bullish') scores.bullish++; else if (r.signal === 'bearish') scores.bearish++; else scores.neutral++; }
  const total = results.length, bullPct = scores.bullish / total, bearPct = scores.bearish / total;
  if (bullPct >= 0.75) return { signal: 'STRONG BUY', strength: 'high', confluence_score: `${scores.bullish}/${total}`, breakdown: scores, advice: 'All/most timeframes bullish with trendline support. Look for 15M entry confirmation.' };
  if (bullPct >= 0.5)  return { signal: 'BUY',        strength: 'medium', confluence_score: `${scores.bullish}/${total}`, breakdown: scores, advice: 'Majority bullish. Wait for 15M bounce off support trendline.' };
  if (bearPct >= 0.75) return { signal: 'STRONG SELL', strength: 'high', confluence_score: `${scores.bearish}/${total}`, breakdown: scores, advice: 'All/most timeframes bearish with trendline resistance. Look for 15M entry confirmation.' };
  if (bearPct >= 0.5)  return { signal: 'SELL',        strength: 'medium', confluence_score: `${scores.bearish}/${total}`, breakdown: scores, advice: 'Majority bearish. Wait for 15M breakdown below support trendline.' };
  return { signal: 'WAIT', strength: 'low', confluence_score: `0/${total}`, breakdown: scores, advice: 'Timeframes mixed. No clear confluence — stand aside until alignment improves.' };
}
...
export function registerRulesTools(server) {
  ...
  }, async ({ symbol, timeframes = ['1D','4H','1H','15M'], bar_count = 300 }) => {
    try {
      ...
      return jsonResult({ success: true, symbol: symbol || 'current chart', verdict: verdict.signal, strength: verdict.strength, confluence_score: verdict.confluence_score, advice: verdict.advice, no_trade_warning: noTrade.warnings?.length ? noTrade.warnings : null, timeframe_breakdown: summary, raw: results });
    } catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
